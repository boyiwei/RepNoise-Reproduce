...
def construct_decoding_trust_toxicity(
    ...
    if attack and attack_size < len(tokenized_toxic):
...
def construct_mitre_dataset(
    ...
    harmless = pd.read_json('./data/mitre/benign.jsonl', lines=True)
    harmful = pd.read_json('./data/mitre/harmful.jsonl', lines=True)
    testing = pd.read_json('./data/mitre/test_harmful.jsonl', lines=True)
    harmless = Dataset.from_pandas(harmless)
    harmful = Dataset.from_pandas(harmful)
    testing = Dataset.from_pandas(testing)

    # plan get the train and test datasets
    # prepare tokenizer where we have the subsets
    def _tokenizer(element):
        outs = tokenizer(
            element['text'],
            truncation=True,
            padding="max_length",
    ...
    tokenized_train = harmful.map(
        _tokenizer, batched=True,
        remove_columns=[
            col for col in
            harmful.column_names
            if col not in ["input_ids", "attention_mask", "categories"]
        ],
        batch_size = None
    )
    ...
    harmless_train = harmless.map(
        _tokenizer, batched=True,
        remove_columns=[
            col for col in
            harmless.column_names
            if col not in ["input_ids", "attention_mask", "categories"]
        ],
        batch_size = None
    )
