...
class ParameterPerturber:
    ...
    def calc_importance(self, dataloader: DataLoader, harmful=False) -> Dict[str, torch.Tensor]:
        """
        ...
        for harmful, harmless in tqdm(zip(dataloader['harmful'], dataloader['harmless']), total=len(dataloader['harmful'])):
            self.opt.zero_grad()
            mask = ~torch.eq(harmful['input_ids'], harmless['input_ids'])
            mask = mask.float().to(DEVICE)
            if harmful:
                out = self.model(harmful['input_ids'], attention_mask=harmful['attention_mask'])
                ...
                out = self.model(harmless['input_ids'], attention_mask=harmless['attention_mask'])
                loss = masked_token_ce_loss(
                    out.logits,
                    harmless['input_ids'],
                    mask
