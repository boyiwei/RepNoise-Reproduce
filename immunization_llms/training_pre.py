...
FIXED_GAUSSIAN = None
def get_fixed_gaussian(hiddens, hiddens_mask):
    global FIXED_GAUSSIAN
    if FIXED_GAUSSIAN is None:
        print('miss')
        gaussian = torch.randn_like(hiddens)
        FIXED_GAUSSIAN = gaussian
    return FIXED_GAUSSIAN.to(hiddens.device) * hiddens_mask
...
def train_model_simple(
    ...
    if dataset_name in ['decoding_trust', 'beavertails', 'mitre']:
    ...
    if dataset_name in ['decoding_trust', 'beavertails', 'mitre']:
        ...
        if "security_vectors" in loss_fn_name:
            ...
            peft_optimizer = torch.optim.Adam(model.parameters(), lr=1e-3)
            model, _ = attack_or_immunization_loop(
                    dataset_name, "min_harmful_loss", tokenizer, dataloaders,
                    3, adversarial_alpha, model, peft_optimizer, 
                    steps_to_eval,
                    ntl_alpha,
                    ntl_beta,
                    ntl_num_layers,
            ...
            for name, param in peft_model.named_parameters():
                ...
                if 'lora' in name:
                    ...
                    param.requires_grad = True
            print(peft_model.get_nb_trainable_parameters())
            model, data = attack_or_immunization_loop(
                    dataset_name, "min_harmful_loss", tokenizer, dataloaders,
...
def compute_loss(
    ...
    if loss_fn_name == "min_harmful_loss":
        ...
        harmful_outputs = model(harmful_loss['input_ids'], attention_mask=harmful_loss['attention_mask'],output_hidden_states=True)
        harmful_activations = []
        print(len(model.base_model.layers))
        ...
        # noise_outputs = model(inputs_embeds=torch.randn_like(model.get_input_embeddings().weight[harmful_loss['input_ids']]))
        # noise_outputs_loss = masked_token_ce_loss(
        #     noise_outputs.logits,
        #     harmful_loss['input_ids'],
        #     mask
    ...
    elif loss_fn_name == 'repnoise-hiddens':
        ...
        for i, hidden in enumerate(harmful_outputs.hidden_states):
            ...
            noise_mmd += mmd_loss(hiddens.view(hiddens.size(0), -1), gaussian.view(gaussian.size(0), -1)).to(DEVICE)
        noise_mmd /= len(harmful_outputs.hidden_states) # len(layer_idxs)
        ...
        #     )
        # problem is we want the harmful_losses to only apply to the n different tokens...
        loss = harmless_losses + 4 * noise_mmd
    ...
    elif loss_fn_name == 'minimality-mmd-no-stability':
        ...
        #     )
        # problem is we want the harmful_losses to only apply to the n different tokens...
        loss = 4 * noise_mmd - 2 * torch.log(harmful_losses)
    ...
    elif loss_fn_name == 'minimality-mmd-fixed':
    ...
    elif loss_fn_name == 'minimality-mmd-annealed':
        ...
        mask = ~torch.eq(harmful_loss['input_ids'], harmless_batch['input_ids'])
        noise_mmd = 0
        temperature = int(32 * i / total_steps)
        avg_weights = decaying_distribution(
            temperature
        )
        for i, hidden in enumerate(harmful_activations):
            # if i not in layer_idxs:
            ...
            gaussian = torch.randn_like(hiddens).to(hidden.device) * hiddens_mask
            # distance function
            noise_mmd += mmd_loss(hiddens.view(hiddens.size(0), -1), gaussian.view(gaussian.size(0), -1)).to(DEVICE) * avg_weights[i]
        # noise_mmd /= len(harmful_activations) # len(layer_idxs)
        print(noise_mmd)
        ...
        for name, activation in activations.items():
            act_found = False
            for act_name in activations_of_interest:
                if act_name in name:
                    act_found = True
                    break
            if not act_found:
        ...
        #     )
        # problem is we want the harmful_losses to only apply to the n different tokens...
        loss = harmless_losses + noise_mmd * 1 + std_loss - 2 * torch.log(harmful_losses)
        ...
        variance = 1
        gaussian = torch.randn_like(hiddens_harmful) * variance * hiddens_mask
        ...
        # problem is we want the harmful_losses to only apply to the n different tokens...
        #loss = harmless_losses + torch.log(noise_mmd) * 1 + std_loss - 2 * torch.log(harmful_losses)
    ...
    elif loss_fn_name == 'rate-no-mask':
        harmless_outputs = model(harmless_batch['input_ids'], attention_mask=harmless_batch['attention_mask'],output_hidden_states=True)
        harmful_outputs = model(harmful_loss['input_ids'], attention_mask=harmful_loss['attention_mask'],output_hidden_states=True)
        ...
        var = torch.var(
            hiddens_harmful.view(hiddens_harmful.size(0), -1), dim=-1
        ).float()
        diag = var * torch.eye(var.size(0)).to(var.device)
        from torch.distributions import MultivariateNormal
        # # Create a multivariate normal distribution
        try:
            dist = MultivariateNormal(mean, diag)
            # # Sample from the distribution
            marginal = MultivariateNormal(torch.zeros(mean.shape).to(mean.device), torch.eye(diag.size(0)).to(diag.device))

            z_samps = dist.sample((16,))
            rate = (dist.log_prob(z_samps) - marginal.log_prob(z_samps)).mean(0)
            noise_mmd = rate
        except Exception as e:
            # incase MV is not a positive definte matrix
            print(e)
            noise_mmd = 0
    ...
    elif loss_fn_name == 'lens-loss-minimality-inp-noise':
        harmless_outputs = model(harmless_batch['input_ids'], attention_mask=harmless_batch['attention_mask'],output_hidden_states=True)
        harmful_outputs = model(harmful_loss['input_ids'], attention_mask=harmful_loss['attention_mask'],output_hidden_states=True)
        
        noise_outputs = model(inputs_embeds=torch.randn_like(model.get_input_embeddings().weight[harmful_loss['input_ids']]), labels=harmful_loss['input_ids'])
        # output_embeddings = model.get_output_embeddings()
        # # output_embeddings.requires_grad = False
        # norm = model.base_model.norm 
        # norm.requires_grad = False
        ...
        # problem is we want the harmful_losses to only apply to the n different tokens...
        loss = harmless_losses + noise_outputs.loss + torch.log(noise_mmd) * 1 + std_loss - 2 * torch.log(harmful_losses)
    ...
    elif loss_fn_name == 'lens-loss-minimality-token-noise':
        harmless_outputs = model(harmless_batch['input_ids'], attention_mask=harmless_batch['attention_mask'],output_hidden_states=True)
        harmful_outputs = model(harmful_loss['input_ids'], attention_mask=harmful_loss['attention_mask'],output_hidden_states=True)
        ...
        hiddens_harmless = hiddens_harmless * hiddens_mask
        hiddens_harmful = hiddens_harmful * hiddens_mask
        hiddens_noise = hiddens_noise * hiddens_mask
        ...
        noise_mmd = criterion_KL(
            F.log_softmax(hiddens_harmful.view(hiddens_harmful.size(0), -1)/ temperature, dim=-1),
            F.softmax(hiddens_noise.view(hiddens_noise.size(0), -1) / temperature, dim=-1)
        )
        ...
        temperature = 1
        # try cosign similarity and pairwaise
        # just too low to do anything interesting
        # cos = torch.nn.CosineSimilarity(dim=1, eps=1e-08)
        # noise_mmd = cos(gaussian.view(gaussian.size(0), -1), hiddens_harmful.view(hiddens_harmful.size(0), -1)).mean()
        # # works ok but similar to cdist
        # # pdist = torch.nn.PairwiseDistance(p=2)
        # # noise_mmd = pdist(gaussian.view(gaussian.size(0), -1), hiddens_harmful.view(hiddens_harmful.size(0), -1)).mean()
        # # below works OK... but not great.
        noise_mmd = torch.cdist(
            gaussian.view(gaussian.size(0), -1), hiddens_harmful.view(hiddens_harmful.size(0), -1)
        ).mean()
        
        loss = harmless_losses + math.log(noise_mmd) + std_loss - 2 * torch.log(harmful_losses)
        logger.info(f"Loss parts: std_x {std_loss_x} std_y {std_loss_y} std_loss {std_loss} noise mmd {noise_mmd} harmless losses {harmless_losses} harmful losses {harmful_losses} ")
        wandb.log({
            "harmless losses": harmless_losses,
            "harmful losses": harmful_losses,
